...
# Querying SLQ
To see all the results for a given table in a sql table we can use the following command
```sql 
SELECT * FROM logins ORDER BY password;
```
The `SELECT *` functions to pick all the rows that contain the certain condition that is specified by `FROM logins`
`ORDER BY` functions to order these results based off the 